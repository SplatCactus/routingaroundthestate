...
/* ============================================================
   QR CODE GENERATION
   Generates a QR code for the campaign URL and injects it into
   both placeholder slots on the page.
   ============================================================ */
(function () {
  var QR_URL = 'https://splatcactus.github.io/routingaroundthestate/';
  var slots = [
    document.getElementById('qr-donate'),
    document.getElementById('qr-float')
  ];
  ...
  slots.forEach(function (el) {
    ...
    qr.addData(QR_URL);
    qr.make();
    /* createSvgTag(cellSize, margin, alt) — clean SVG, no table fallback */
    el.innerHTML = qr.createSvgTag(4, 0);
    /* Make the SVG fill the container and respect theme colors */
    var svg = el.querySelector('svg');
    if (svg) {
      svg.style.width = '100%';
      svg.style.height = 'auto';
      svg.style.display = 'block';
      /* Invert in dark mode: the library outputs black-on-white,
         which clashes with the dark background. Use CSS filter. */
